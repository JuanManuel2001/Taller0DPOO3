...
public class ConsolaOlimpicos
{
		...
		while (continuar)
		{
			try
			{
				...
				if (opcion_seleccionada == 1)
					...
					//ejecutarAtletaPais();
					ejecutarPaisAlQuePerteneceAtleta();
	...
	private void ejecutarAtletasConMasMedallasQue()
	{
		...
		{
			System.out.println("El nÃºmero ingresado no es válido. Por favor escriba un número entero.");
		}
	}
	...
	private void ejecutarPaisAlQuePerteneceAtleta()
	{
		System.out.println("\n" + "País al que pertenece un atleta" + "\n");
		String nombreAtleta = input("Ingrese el nombre del atleta del que desea conocer el país al que pertenece");
		String resultado=calculadora.atletaPertenecePais(nombreAtleta);
		if (resultado.length()==0) {
			System.out.println("No se encontró al atleta "+nombreAtleta);
		}else {
			System.out.println("El atleta "+nombreAtleta+" pertenece al país "+resultado);
		}
